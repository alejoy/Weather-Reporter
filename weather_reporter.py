...
def llamar_api_directa(modelo, prompt):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"⚠️ Cuota llena en {modelo} (429). Saltando al siguiente...")
            return None
...
def generar_noticia_robusta(prompt):
    # LISTA FIJA DE PRIORIDAD (CASCADA)
    # 1. Flash: El rey de la cuenta gratuita. Rápido y sin límites estrictos.
...
def main():
    ...
    # Fallback FINAL (Solo si fallan los 3 modelos)
    if not texto_ia:
        print("❌ ERROR CRÍTICO: Fallaron todos los modelos.")
    ...
    # Intentar detectar si la IA puso un título al principio
